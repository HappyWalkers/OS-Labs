...
sys_page_alloc(envid_t envid, void *va, int perm)
{
    ...
    if((r = page_insert(env->env_pgdir, pageInfo, va, perm) < 0)) {
        page_free(pageInfo);
        return r;
    }
//    cprintf("sys_page_alloc: envid = %08x, pa = %08x, va = %08x, perm = %08x\n", env->env_id, page2pa(pageInfo), va, perm);
    return 0;
}
	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
    ...
    if((r = page_insert(dstenv->env_pgdir, pageInfo, dstva, perm) < 0)) {
        return r;
    }
    return 0;
}
...
sys_ipc_try_send(envid_t envid, uint32_t value, void *srcva, unsigned perm)
{
    ...
    if((uint32_t)srcva < UTOP) {
        ...
        pte_t *pte;
        struct PageInfo *pageInfo = page_lookup(curenv->env_pgdir, srcva, &pte);
        if(pageInfo == NULL) {
            cprintf("sys_ipc_try_send: srcva is not mapped in the caller's address space\n");
            return -E_INVAL;
        }
        if((perm & PTE_W) && !(*pte & PTE_W)) {
            cprintf("sys_ipc_try_send: srcva is read-only in the current environment's address space\n");
            return -E_INVAL;
        }
        if(env->env_ipc_dstva != NULL && (r = page_insert(env->env_pgdir, pageInfo, env->env_ipc_dstva, perm) < 0)) {
...
// transmit a package from user space
static int
sys_transmit_packet(void *packet, size_t len)
{
